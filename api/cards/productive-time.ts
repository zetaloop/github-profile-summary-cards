...
export default async (req: VercelRequest, res: VercelResponse) => {
    const { username, theme = 'default', utcOffset = '0', no_bg = 'false', no_stroke = 'false'} = req.query;
    ...
    try {
        ...
        while (true) {
            try {
                const cardSVG = await getProductiveTimeSVGWithThemeName(username, theme, Number(utcOffset), no_bg === 'true', no_stroke === 'true');
